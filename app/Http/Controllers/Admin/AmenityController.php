...
class AmenityController extends Controller
{
    ...
    public function __construct()
    {
          ...
          $this->single_heading = "Amenity";
          $this->route->list = route('admin.amenity.list');
          $this->route->add = route('admin.amenity.add');
          $this->route->store = route('admin.amenity.store');
          $this->route->status = route('admin.amenity.status');
          $this->route->edit = route('admin.amenity.edit',':id');
          $this->route->destroy = route('admin.amenity.destroy',':id');
          $this->route->saveimage = route('admin.amenity.saveimage');
    }

    public function index()
    {
           return view('admin.amenity.index',['route'=>$this->route, 'single_heading'=>$this->single_heading]);
    }
    ...
    public function list()
    {
        ...
        $qry = Amenity::orderBy($columnName, $columnSortOrder)->where('title', 'LIKE', '%' . $searchValue . '%') ->orWhere('description', 'LIKE', '%' . $searchValue . '%');
        $result = $qry->get();
...
//  echo "<pre>"; print_r($row->file_id->image);die;
              ...
              $action .= '</div>';
                        ...
                        $file='';
                        if ($row->photo) {
                            $file = '<img src="' . asset('uploads/amenity/' . @$row->photo->file) . '" class="img-fluid table-image" alt="" width="50" height="50" >';
                        }
    ...
    {
        return view('admin.amenity.add',['route'=> $this->route , 'single_heading'=> $this->single_heading]);
    }
    ...
    public function status(Request $request)
    {
        ...
        $status = Amenity::find($request->id);
    ...
    public function edit($id)
    {
        ...
        return view('admin.amenity.edit',['route'=>$this->route,'single_heading'=>$this->single_heading, 'info'=>$info]);
    }
    ...
    public function update(Request $request)
    {
        $validator = Validator::make(
          ...
          if($validator->fails())
          {
            ...
            $info= Amenity::find($request->id)->fill([
                'title'=>$request->title,
                'icon'=>$request->file_id,
                'description'=>$request->description,
                'status'=>$request->status,
